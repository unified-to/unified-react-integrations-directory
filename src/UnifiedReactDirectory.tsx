...
export default function UnifiedDirectory(props: UnifiedDirectoryProps) {
  ...
  useEffect(() => {
    if (!loading && !INTEGRATIONS.length) {
      ...
      load_data().then((data: IIntegration[]) => {
        data =
          data.filter(
            (i) => !(i.categories.length === 1 && i.categories[0] === "auth")
          ) || [];
  ...
  function filter(integrations: IIntegration[]) {
    return (
      integrations?.filter(
        (integration) =>
          !selectedCategory ||
          integration.categories.indexOf(selectedCategory) !== -1
      ) || []
    );
  }

  function unified_get_auth_url(integration: IIntegration) {
    let url = `${API_URL}/unified/integration/auth/${
      props.workspaceId || props.workspace_id
  ...
  async function load_data() {
    const url = `${API_URL}/unified/integration/workspace/${
      props.workspaceId || props.workspace_id
    }?summary=1${
      ...
      props.environment === "Production" || !props.environment
        ? ""
        : "&env=" + encodeURIComponent(props.environment)
    }`;
  ...
  return (
    <div className="unified">
      {!props.nostyle && (
        <style>@import url(https://api.unified.to/docs/unified.css)</style>
      )}
      {!props.notabs &&
        CATEGORIES &&
        CATEGORIES.length > 0 &&
        filter(INTEGRATIONS).length && (
          <div className="unified_menu">
            <button
              className={`unified_button unified_button_all ${
            ...
            {CATEGORIES.map((cat) => (
              <button
                key={cat}
                className={`unified_button unified_button_${cat} ${
      ...
      <div className="unified_vendors">
        ...
        {filter(INTEGRATIONS).length === 0 && (
          <div className="unified_vendor">No integrations available</div>
        )}
      </div>
    </div>
  );
