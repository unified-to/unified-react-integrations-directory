...
export const schemaConnection = {
	...
	categories: { type: [ String ], enum: [ 'passthrough', 'hris', 'ats', 'auth', 'saml', 'crm', 'enrich', 'martech', 'ticketing', 'uc', 'accounting', 'storage', 'commerce', 'payment', 'genai', 'messaging', 'kms', 'task', 'scim', 'lms', 'repo', 'metadata', 'calendar', 'verification', 'ads', 'analytics', 'forms', 'shipping', 'assessment', 'signing', 'clubs', 'datastore', 'cdp', 'performance', 'social' ], index: true }, // The Integration categories that this connection supports
	...
	last_unhealthy_code: { type: String }, // The HTTP status code that caused the last unhealthy status
...
export const schemaIntegrationSupport = {
	...
	list_assigned_user_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	...
	list_template_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	virtual_webhook_template_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	native_webhook_template_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	native_webhook_list_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	list_reference: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	virtual_webhook_reference: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	virtual_webhook_account_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	list_database_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	virtual_webhook_database_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	native_webhook_database_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	list_table_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	virtual_webhook_table_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	native_webhook_table_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
	...
	list_profile_id: { type: String, enum: [ 'supported-required', 'supported', 'not-supported' ] },
...
export const schemaWebhook = {
	...
	filters: { type: Object },
	db_type: { type: String, enum: [ 'mongodb', 'mysql', 'postgres', 'mssql', 'mariadb', 'supabase', 'snowflake' ] },
	db_url: { type: String },
...
export const schemaWorkspace = {
	...
	default_secretsmanager_id: { type: SchemaTypes.ObjectId, ref: 'SecretsManager' }, // Default SecretsManager document id for new credentials
	default_secretsmanager_env_ids: { type: Object, ref: 'SecretsManager' }, // Environment name to SecretsManager document id
};
...
export const schemaWorkspaceIntegrationAuth = {
	...
	key: { type: String }, // the private KEY X.509 certificate in Base64 ASCII format
	dev_api_key: { type: String },
	saml_idp_url: { type: String }, // the SAML IdP SSO/login URL for end-user SAML login
	saml_idp_entity_id: { type: String }, // the SAML IdP issuer/entity ID
	saml_pem: { type: String }, // the SAML IdP signing certificate; PEM X.509 in Base64 ASCII format
	needs_pkce: { type: Boolean }, // opt-in PKCE for OAuth2 when the integration app or org requires it
};
...
export const schemaWorkspaceIntegration = {
	...
	auth_type: { type: String, enum: [ 'oauth2', 'other', 'token' ] }, // the authentication method to use for this workspace integration; when set, it overrides the default auth-method selection (e.g. force 'token' even when Unified OAUTH credentials exist)
	environment: { type: String, default: 'Production' }, // authentication environment
	categories: { type: [ String ], enum: [ 'passthrough', 'hris', 'ats', 'auth', 'saml', 'crm', 'enrich', 'martech', 'ticketing', 'uc', 'accounting', 'storage', 'commerce', 'payment', 'genai', 'messaging', 'kms', 'task', 'scim', 'lms', 'repo', 'metadata', 'calendar', 'verification', 'ads', 'analytics', 'forms', 'shipping', 'assessment', 'signing', 'clubs', 'datastore', 'cdp', 'performance', 'social' ] },
